...
function createSnapshot(iterator) {
  // TODO(sorvell): if there's a native TemplateElement, we may not have MDV,
  // but in this case, the template's dom is already stored in.content
  var templateElement = (iterator.templateElement && 
    iterator.templateElement.content) || iterator.templateElement;
...
function inDocument(node) {
  // TODO(sorvell): doesn't work with shadowDOM
  return true;
...
function TemplateInstance(iterator, templateScope) {
  ...
  // TODO(sorvell): enable giving model directly to a template by making 
  // instances bind to the template
  //var parentNode = iterator.templateElement.parentNode;
  var parentNode = iterator.templateElement;
...
TemplateInstance.prototype = createObject({
  ...
  createDom_: function() {
    ...
    for (var i = 0; clone.hasChildNodes(); i++) {
      ...
      // TODO(sorvell): facilitate getting model from a template element
      // by set sourceTemplate_ to the iterator's template element.
      if (this.templateIterator) {
        node.sourceTemplate_ = this.templateIterator.templateElement;
      }
