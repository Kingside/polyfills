...
function setModel(model) {
  ...
  // TODO(sorvell): ad hoc but is convenient to dirty check after setting model...
  if (window.Model) {
    Model.dirtyCheck();
  }
}
...
function getComputedModel() {
  ...
  if (this.model_ !== undefined)
    model = this.model_;
  // TODO(sorvell): we need iterated template nodes to get correct model
  // when template is given a model. Use 'sourceTemplate_' property 
  // to find relevant template
  else if (this.sourceTemplate_) {
    model = this.sourceTemplate_.computedModel;
  }
...
var modelDescriptor = {
  configurable: true,
  enumerable: true,
  // TODO(sorvell): native design for model is polyfill's computedModel; not
  // sure just using computed model here is kosher.
  get: getComputedModel,
  set: setModel,
};
